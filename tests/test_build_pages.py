...
class BuildPagesTest(unittest.TestCase):
    def test_builds_index_and_weekly_report_pages(self):
        ...
        try:
            ...
            self.assertIn(root / "docs" / "admin-auth.js", written)
            ...
            admin_auth = (root / "docs" / "admin-auth.js").read_text(encoding="utf-8")
            auth_pages = [
                (root / "docs" / name).read_text(encoding="utf-8")
                for name in ("admin.html", "subscriptions.html", "jobs.html", "job.html")
            ]
            ...
            for page_html in auth_pages:
                self.assertIn('<meta name="referrer" content="no-referrer">', page_html)
                self.assertIn('<script src="admin-auth.js"></script>', page_html)
                self.assertIn("data-admin-auth-mount", page_html)
                self.assertNotIn('params.get("admin_token")', page_html)
                self.assertNotIn('localStorage.getItem("github_weekly_admin_token")', page_html)
                self.assertNotIn('localStorage.setItem("github_weekly_admin_token")', page_html)
            self.assertIn('type="password"', admin_auth)
            self.assertIn('autocomplete="off"', admin_auth)
            self.assertIn('"X-Admin-Token": token', admin_auth)
            ...
            self.assertIn("evidence_coverage", admin_page)
            ...
            self.assertIn("React 项目研究工作台", agent_page)
            self.assertIn('const page = "agent"', agent_page)
            self.assertIn('const target = "app/#" + route', agent_page)
            self.assertIn('window.location.replace(target)', agent_page)
            ...
            self.assertIn('const page = "explore"', explorer)
            self.assertIn('const page = "recommendations"', recommendations)
            self.assertIn('window.location.replace(target)', explorer)
            self.assertIn('window.location.replace(target)', recommendations)
            ...
            self.assertIn('const page = "compare"', compare_page)
            self.assertIn('window.location.replace(target)', compare_page)
            ...
            self.assertIn('const page = "project"', project_page)
            self.assertIn('params.get("repo")', project_page)
            self.assertIn('route = "/projects/"', project_page)
            self.assertIn('window.location.replace(target)', project_page)
