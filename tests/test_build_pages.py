...
class BuildPagesTest(unittest.TestCase):
    def test_builds_index_and_weekly_report_pages(self):
        ...
        try:
            ...
            self.assertIn(root / "docs" / "agent.html", written)
            ...
            admin_html = (root / "docs" / "admin.html").read_text(encoding="utf-8")
            project_html = (root / "docs" / "project.html").read_text(encoding="utf-8")
            self.assertIn("事件订阅与推送候选", admin_html)
            self.assertIn("/v1/notification-candidates", admin_html)
            self.assertIn("retry_failed", admin_html)
            self.assertIn("项目事件与订阅", project_html)
            self.assertIn("订阅项目变化", project_html)
            self.assertIn("notification_memory", project_html)
            self.assertIn("feedbackHttpError", project_html)
            self.assertIn("feedbackErrorMessage", project_html)
            self.assertIn("ADMIN_API_TOKEN", project_html)
            ...
            self.assertIn("[项目匹配 Agent](agent.html)", index)
            ...
            agent_page = (root / "docs" / "agent.html").read_text(encoding="utf-8")
            ...
            self.assertIn("RAG 对话", admin_page)
            self.assertIn("rag-chat-shell", admin_page)
            self.assertIn("rag-chat-messages", admin_page)
            self.assertIn("rag-chat-bubble user", admin_page)
            self.assertIn("rag-chat-bubble assistant", admin_page)
            self.assertIn("rag-chat-composer", admin_page)
            self.assertIn('id="ragChatQuery"', admin_page)
            self.assertIn('id="ragChatMode"', admin_page)
            self.assertIn('id="ragChatLimit"', admin_page)
            self.assertIn('id="ragChatLanguage"', admin_page)
            self.assertIn('id="ragChatCategory"', admin_page)
            self.assertIn('id="ragChatSource"', admin_page)
            self.assertIn('id="sendRagChat"', admin_page)
            self.assertIn('id="clearRagChat"', admin_page)
            self.assertIn('id="ragChatMessages"', admin_page)
            self.assertIn("setupRagChat", admin_page)
            self.assertIn("sendRagChatMessage", admin_page)
            self.assertIn("renderRagChat", admin_page)
            self.assertIn("ragChatMessageHtml", admin_page)
            self.assertIn("loadRagChatHistory", admin_page)
            self.assertIn("saveRagChatHistory", admin_page)
            self.assertIn("github_weekly_rag_chat_history", admin_page)
            self.assertIn("answer_quality", admin_page)
            ...
            self.assertIn('id="askDevContext"', admin_page)
            self.assertIn('id="runDevContextSearch"', admin_page)
            self.assertIn('id="devContextJobs"', admin_page)
            ...
            self.assertIn("/v1/dev-context/ask", admin_page)
            ...
            self.assertIn("askDevContext", admin_page)
            self.assertIn("runDevContextSearch", admin_page)
            self.assertIn("devContextSearchHtml", admin_page)
            self.assertIn("devContextAskHtml", admin_page)
            self.assertIn("devContextJobsHtml", admin_page)
            self.assertIn("dev_context_index", admin_page)
            self.assertIn("feedbackSummaryHtml", admin_page)
            self.assertIn("GitHub 项目研究 Agent", agent_page)
            self.assertIn('id="agentMatchQuery"', agent_page)
            self.assertIn('id="agentMatchMode"', agent_page)
            self.assertIn('id="agentMatchLimit"', agent_page)
            self.assertIn('id="agentMatchLanguage"', agent_page)
            self.assertIn('id="agentMatchCategory"', agent_page)
            self.assertIn('id="agentMatchSource"', agent_page)
            self.assertIn('id="sendAgentMatch"', agent_page)
            self.assertIn('id="clearAgentMatch"', agent_page)
            self.assertIn('id="agentMatchMessages"', agent_page)
            self.assertIn("setupAgentMatch", agent_page)
            self.assertIn("sendAgentMatchMessage", agent_page)
            self.assertIn("renderAgentMatch", agent_page)
            self.assertIn("agentMatchMessageHtml", agent_page)
            self.assertIn("projectMatchCardsHtml", agent_page)
            self.assertIn("loadAgentMatchHistory", agent_page)
            self.assertIn("saveAgentMatchHistory", agent_page)
            self.assertIn("github_weekly_agent_match_history", agent_page)
            self.assertIn("/v1/rag/ask", agent_page)
            self.assertIn("answer_quality", agent_page)
            self.assertIn("fallback_reason", agent_page)
            self.assertIn("prompt_context", agent_page)
            self.assertIn("project.html?", agent_page)
            self.assertIn("compare.html?", agent_page)
            ...
            self.assertIn('value="rag_search_evaluation"', jobs_page)
            self.assertIn('value="dev_context_index"', jobs_page)
            ...
            self.assertIn("feedbackHttpError", recommendations)
            self.assertIn("feedbackErrorMessage", recommendations)
            self.assertIn("ADMIN_API_TOKEN", recommendations)
            self.assertIn("feedback_memory", recommendations)
            self.assertIn("preference_score", recommendations)
            self.assertIn("recommendation_score", recommendations)
            self.assertIn("ranking_factors", recommendations)
            self.assertIn("recommendation_reason", recommendations)
            self.assertIn("rag_reason", recommendations)
            self.assertIn("feedback_reason", recommendations)
            ...
            self.assertIn("反馈影响推荐", admin)
            self.assertIn("/v1/recommendations?limit=20", admin)
            ...
            self.assertIn("Agent 研究摘要", project_page)
            self.assertIn("projectProfileHtml", project_page)
            self.assertIn("project_profile", project_page)
            self.assertIn("Agent 下一步动作", project_page)
            self.assertIn("agentTasksHtml", project_page)
            self.assertIn("createProjectAgentTask", project_page)
            self.assertIn("inspectProjectAgentTask", project_page)
            self.assertIn("executeProjectAgentTask", project_page)
            self.assertIn("execution-check", project_page)
            self.assertIn("agent_task_runs", project_page)
            self.assertIn("/v1/agent-tasks/", project_page)
            ...
            self.assertIn("项目 Agent 任务", admin_page)
            self.assertIn("loadProjectAgentTasks", admin_page)
            self.assertIn("/v1/agent-tasks?limit=100", admin_page)
            self.assertIn("/v1/agent-task-runs?limit=30", admin_page)
            self.assertIn("执行成功", admin_page)
