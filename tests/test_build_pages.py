...
class BuildPagesTest(unittest.TestCase):
    def test_builds_index_and_weekly_report_pages(self):
        ...
        try:
            ...
            self.assertIn('id="taskDryRun"', admin_page)
            self.assertIn('id="taskConfirmDelivery"', admin_page)
            ...
            self.assertIn("/v1/rag/retrieve?", project_page)
            self.assertIn("RAG 解释历史", project_page)
            self.assertIn("/v1/rag/explanations?", project_page)
            self.assertIn('historyParams.set("repo", repo)', project_page)
