...
Ask 现在返回当前归档内的可审计 `recommendations[]`：后端按仓库聚合本轮证据，返回相对匹配分、显式 `language/category/source` 约束状态、理由和证据映射。只有质量闸门通过、`data_freshness=fresh` 且第一项 `eligibility=eligible` 时，React 才显示“当前归档内最匹配候选”；否则显示“暂无可确认首选”。`python scripts/evaluate_project_recommendations.py` 使用同一 52 条评估集输出三种检索模式的 Top-1、Recall@3、MRR@10、硬约束违反率和无首选率。
