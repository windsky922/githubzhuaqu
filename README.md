...
POST Ask 支持无状态追问和自然语言硬约束。resume/refine 在 SQL 或向量候选读取阶段限制到上一轮仓库，明确“重新找”才扩大到全归档。language、license、category、source 和 tech_stack 只由确定性元数据验证；deployment、cost 只接受非 `model_enrichment` 清洗 chunk 的明确命中。任一冲突为 rejected，无法验证为 unknown；没有 eligible 且全冲突时返回 `answer_mode=no_match`，存在 unknown 时返回 clarification。既有 GET 契约与 SSE 事件顺序保持不变。
