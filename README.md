...
29. GitHub Actions 每次运行后可自动执行一次 RAG 检索质量评估，并在生成 Pages 前把评估任务写入 SQLite jobs，用于长期质量趋势观测。
30. 后端提供 RAG 解释接口 `/v1/rag/explain`，把召回证据整理为推荐解释、证据引用、风险提示和下一步动作，并支持 `fts5`、`vector`、`hybrid` 三种检索模式。
...
34. 后端提供 RAG 解释回填接口 `/v1/rag/backfill-explanations`、计划任务接口 `/v1/rag/backfill-plan` 和维护计划接口 `/v1/rag/maintenance-plan`，可按诊断结果创建语料重建、embedding 构建或解释回填任务；默认 dry-run 预览，只有显式传入 `confirm_execution=true` 才允许写入 SQLite。
35. 后端提供 RAG 维护历史汇总接口 `/v1/rag/maintenance-report`，聚合语料重建、embedding 构建、解释回填和检索评估任务的状态、最近结果、计数变化和维护建议。
36. 后端提供单项目 RAG 聚合接口 `/v1/projects/{owner}/{repo}/rag`，一次返回项目摘要、证据块、引用、prompt_context、解释历史和项目级解释质量摘要。
