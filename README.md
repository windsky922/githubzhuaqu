...
`main` 的每次 push 和所有 pull request 都会运行独立提交质量检查：Python 3.12 全量测试与安全检查、Node.js 22 前端类型检查/单元测试/生产构建、`docs/app` 构建产物一致性，以及 Chromium 桌面/手机 Playwright 回归。浏览器测试使用本地固定 mock，不读取业务 Secrets，不运行采集、模型或真实推送。可在本地运行 `npm run test:e2e`。
...
最新 V2 对抗性审查、主要遗漏与整改优先级见：[GitHub 项目研究 Agent V2 审查与路线图](docs/project-review-agent-v2-roadmap.md)。
...
`evals/constraint_parsing_cases.jsonl` 保存 100 条分句级约束样本，固定划分为 60 条 development、20 条 locked regression 和 20 条 adversarial；`evals/constraint_evidence_cases.jsonl` 另存 36 条部署/成本句子证据。运行 `python scripts/evaluate_constraint_parsing.py` 会同时输出约束精确匹配、operator、澄清、证据状态准确率、错误合格率和硬约束违反率；离线运行使用 `deployment=offline`，不会再与普通本地部署混为一类。
...
POST Ask 支持无状态追问和自然语言硬约束。resume/refine 在 SQL 或向量候选读取阶段限制到上一轮仓库，明确“重新找”才扩大到全归档。约束解析按分句确定否定作用域，不能表示的冲突或析取关系会先澄清；`deployment` 区分 `local/offline/cloud`。language、license、category、source 和 tech_stack 只由确定性元数据验证；deployment、cost 只接受非 `model_enrichment` 清洗 chunk 的句子级证据，并区分支持、明确冲突、条件成立、仅试用和外部依赖。免费试用不等于免费，self-hosted UI 加托管推理不等于完全离线。任一冲突为 rejected，条件不完整或证据不足为 unknown；没有 eligible 且全冲突时返回 `answer_mode=no_match`，存在 unknown 时返回 clarification。既有 GET/POST 契约与 SSE 事件顺序保持不变。
