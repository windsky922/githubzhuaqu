...
29. 后端提供 RAG 解释接口 `/v1/rag/explain`，把召回证据整理为推荐解释、证据引用、风险提示和下一步动作，并支持 `fts5`、`vector`、`hybrid` 三种检索模式。
30. 后端会把 RAG 解释结果写入 SQLite `rag_explanations`，并通过 `/v1/rag/explanations` 查询历史解释、质量分、引用完整度和指定项目的解释历史。
31. 后端提供 RAG 质量概览接口 `/v1/rag/quality-summary`，汇总解释数量、平均质量分、质量分布和低质量样本。
32. 后端提供 RAG 覆盖缺口接口 `/v1/rag/coverage`，列出缺少证据块、embedding 或解释历史的项目，便于补库和后续 Agent 自动优化。
33. 后端提供 RAG 解释回填接口 `/v1/rag/backfill-explanations`、计划任务接口 `/v1/rag/backfill-plan` 和维护计划接口 `/v1/rag/maintenance-plan`，可按诊断结果创建语料重建、embedding 构建或解释回填任务；默认 dry-run 预览，只有显式传入 `confirm_execution=true` 才允许写入 SQLite。
34. 后端提供 RAG 维护历史汇总接口 `/v1/rag/maintenance-report`，聚合语料重建、embedding 构建、解释回填和检索评估任务的状态、最近结果、计数变化和维护建议。
35. 后端提供单项目 RAG 聚合接口 `/v1/projects/{owner}/{repo}/rag`，一次返回项目摘要、证据块、引用、prompt_context、解释历史和项目级解释质量摘要。
36. 后端提供相似项目候选接口，基于 FTS5 召回，并结合语言、方向、来源、关键词和热度信号生成可解释的相似项目列表。
37. 项目详情页在本地后端或 `api=1` 模式下会展示后端相似项目候选、相似度分和相似原因。
38. 后端提供项目对比接口和前端对比页，可一次比较多个仓库的语言、方向、历史入选次数、Star 增长、Trending 排名、质量分和风险提示。
