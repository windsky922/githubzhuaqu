...
26. 后端提供相似项目候选接口，基于 FTS5 召回，并结合语言、方向、来源、关键词和热度信号生成可解释的相似项目列表。
27. 项目详情页在本地后端或 `api=1` 模式下会展示后端相似项目候选、相似度分和相似原因。
28. 后端提供项目对比接口和前端对比页，可一次比较多个仓库的语言、方向、历史入选次数、Star 增长、Trending 排名、质量分和风险提示。
