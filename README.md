...
41. 推荐接口会读取 `project_feedback`，把正负反馈转换为 `preference_score` 和 `feedback_memory`，让用户反馈开始影响推荐排序。
42. 单项目 RAG 聚合接口会返回 `feedback_memory`，便于项目详情页和后续 Agent 判断该项目是否值得继续跟踪。
43. 项目详情页在本地 API 模式下提供“有用 / 不适合 / 继续跟踪”反馈入口，写入 `/v1/feedback` 并显示项目级反馈记忆。
44. 个性化推荐页会展示 `preference_score` 和反馈记忆摘要，并支持对推荐结果直接提交反馈后刷新排序。
45. 管理页会展示最近反馈记忆汇总，便于检查反馈样本是否已经进入数据库并影响推荐链路。
...
| `src/api/` | 后端 API，用于查询历史项目、运行记录、个性化方向、项目反馈和最新周报 |
