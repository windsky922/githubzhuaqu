...
本地前端现在提供两个 RAG 入口：`admin.html?api=1` 用于管理、诊断和证据检查；`app/#/agent?api=1` 是 React 项目匹配工作台，旧 `agent.html?api=1` 会自动跳转。工作台使用顶部导航、会话历史、移动端抽屉和 GPT 式输入区；候选集合、顺序和首选状态只读取 Ask `final` 的后端 `recommendations[]`，不再从 citations 或 contexts 第一项推断。它默认使用 `/v1/rag/ask/stream?mode=hybrid&limit=3&auto_build=true`，以 SSE 展示“待质量校验草稿”，最终只保留通过证据质量闸门的回答；无证据或质量失败会明确显示 refusal/规则降级。对话只保存在浏览器 `localStorage`，不保存密钥，不新增后端会话。
