...
`evals/constraint_parsing_cases.jsonl` 保存 100 条分句级约束样本，固定划分为 60 条 development、20 条 locked regression 和 20 条 adversarial。运行 `python scripts/evaluate_constraint_parsing.py` 会输出约束精确匹配、operator 和澄清准确率；离线运行使用 `deployment=offline`，不会再与普通本地部署混为一类。
