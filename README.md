...
24. 后端提供项目对比接口，可一次比较多个仓库的语言、方向、历史入选次数、Star 增长、Trending 排名、质量分和风险提示。
...
`compare.html` 是项目对比页，支持 `compare.html?repos=owner/a,owner/b`。本地后端或 URL 带 `api=1` 时会优先读取 `/v1/projects/compare`，否则使用 `projects.json` 静态聚合，展示对比矩阵、领先指标和缺失项目。
