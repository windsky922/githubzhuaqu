...
29. 后端提供 RAG 解释回填接口 `/v1/rag/backfill-explanations` 和计划任务接口 `/v1/rag/backfill-plan`，默认 dry-run 预览，只有显式传入 `confirm_execution=true` 才允许 API 写入 SQLite；RAG 回填可以通过 `/v1/jobs` 追踪并交给本地 runner 执行。
...
| `scripts/run_planned_job.py` | 执行一个 planned 任务，当前支持周报任务和 RAG 回填任务 |
