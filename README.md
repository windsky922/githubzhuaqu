...
29. 后端提供 RAG 解释回填接口 `/v1/rag/backfill-explanations`，默认 dry-run 预览，只有显式传入 `confirm_execution=true` 才允许 API 写入 SQLite；每次 API 调用都会写入 `rag_backfill` 任务和审计事件，方便在 `/v1/jobs` 追踪。
...
该接口会返回 `job_id`，并把本次预览或写库记录到 `/v1/jobs?kind=rag_backfill` 和 `/v1/jobs/{job_id}/events`。
