...
8. Telegram、飞书和企业微信只推送可点击链接，默认包含周报正文、项目筛选和运行状态三个入口，完整内容保存在仓库归档中。
...
12. 提供 FastAPI 只读后端接口，支持历史项目查询、单项目详情聚合、运行记录、个性化方向和最新周报读取。
...
`project.html` 是单项目详情页，支持 `project.html?repo=owner/name`，展示历史入选次数、首次和最近入选日期、累计新增 Star、最好 Trending 排名、质量提示、风险提示、历史入选记录和相似项目。本地后端或 URL 带 `api=1` 时会优先读取 `/api/projects/{owner}/{repo}`，否则使用 `projects.json` 静态聚合。
...
3. `profiles.json` 汇总公开个性化方向，例如方向名称、学习目标、偏好语言和主题关键词。
