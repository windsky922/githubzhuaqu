...
8. Telegram、飞书和企业微信只推送可点击链接，默认包含周报正文和项目筛选两个入口，完整内容保存在仓库归档中。
...
Telegram、飞书和企业微信推送的是 GitHub Pages 上的两个阅读入口：周报正文 `weekly/YYYY-MM-DD.html` 和项目筛选 `explorer.html?date=YYYY-MM-DD`。仓库需要手动启用一次 Pages：
