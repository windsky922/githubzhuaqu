...
管理首页中的 RAG 检索区会调用 `/v1/rag/retrieve` 和 `/v1/rag/vector-search`，用于查看证据块、引用和 `prompt_context`。向量检索会在 `auto_build=true` 时自动构建本地 `local-hash-v1` 索引，也可以先手动运行 `py scripts\build_rag_embeddings.py`。
