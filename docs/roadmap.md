...
后续建设路线调整为“核心功能优先”。项目不再优先投入页面细节、展示装饰和推送渠道末端优化，而是先补齐后端服务化、数据层、任务调度和 Agent/RAG 能力。

优先顺序：

1. 保持每周热点采集、周报生成、归档和推送主链路稳定。
2. 扩展 FastAPI，从只读查询升级到 `/v1/*` 服务接口。
3. 增加最小任务模型，支持手动触发、后台执行和状态查询。
4. 保留 JSON/SQLite，设计 PostgreSQL 与 pgvector 的后续迁移路径。
5. 建设 Agent 分析能力，先做结构化查询、证据检索和推荐解释，再考虑复杂 Agent 框架。
6. 前端继续服务于项目检索、详情、运行状态和 Agent 分析，不优先做重型 SPA 或视觉装饰。

详细方案见 `docs/core-development-plan.md`。
...
5. 基于公共 JSON 设计前端筛选和多渠道入口。公共 JSON 和轻量筛选页基础版本已完成。
6. 增加公共 JSON 和 SQLite 数据契约测试。已完成基础版本。
7. 增加 RSS 订阅输出。已完成基础版本。
