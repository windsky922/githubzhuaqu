...
`assistant_state` 兼容接收 schema-v1，响应固定为 schema-v2：`schema_version`、`revision`、`goal`、`knowledge_context`、`constraints`、`candidate_repository_ids`、`primary_repository_id`、`last_intent`、`pending_question`、`source_identity`、`mode`、`resumable`。`knowledge_context` 只允许 `topic`（最多 200 字符）、`outline`（最多 12 个 `{id,title}`，ID 最多 24 字符、标题最多 120 字符）和必须命中 outline 的 `focus_id`；缺少有效主题或提纲时整体归一为空。它不包含历史 assistant 回答、citations、evidence、sections、`prompt_context`、provider 原始输出、请求头、凭证或写操作确认。客户端回传值不是事实、证据或授权，服务端和前端投影都必须重新校验。
...
7. 项目数组只保留公开仓库元数据、分类/评分/趋势、README 摘要、来源与质量信号；运行摘要只保留公开计数、状态、报告链接、汇总指标和 allowlist 内的 `rag_freshness` schema-v1 字段；趋势嵌套条目只保留名称/计数或公开项目摘要。未知键、查询、note/payload、原始错误详情、运行或状态路径、投递结果及 attestation 未知嵌套键均不得出现在公开投影。
...
13. 私有 blind pack 必须位于仓库外、使用 schema-v2 JSONL、至少包含 20 条冻结样本；各行 `evaluation_date` 必须一致，`id` 与规范化查询必须唯一，`categories` 只能使用固定枚举并满足要求覆盖。`expected.input_route`、`expected.data_freshness`、`expected.evidence_coverage`、`expected.acceptable_primary_ids`、`expected.relevant_repository_ids` 和 `expected.candidate_eligibility` 是 route、source 状态、证据覆盖、首选、Recall@3、候选资格与硬约束违规的独立标签，不得从响应反推；freshness、clarification、no-match、unknown/rejected 类别必须与这些标签一致，首选 ID 必须在 eligibility map 中标为 `eligible`。runner 绑定 `GITHUB_WEEKLY_SNAPSHOT_ROOT`、冻结 freshness 日期、禁用模型客户端，并仅在临时 SQLite 运行 contextual normal/SSE 服务路径；pack SHA-256 与 snapshot SHA-256 在执行前后必须一致，后者只覆盖 `data/raw`、`data/runs`、`data/selected`、`data/trends` 下的 JSON。单次 baseline 只能绑定一种实际 source freshness；fresh 与 stale 必须分别运行。输出必须是仓库、pack 和 snapshot 之外的全新普通文件。baseline 只保存样本数、冻结日期、聚合类别、聚合指标、每项指标分母和固定失败类别；非 runner `ValueError` 统一映射为固定 runtime error，不保存题目、标签、回答、仓库级失败、路径或阈值，也不得进入公开 fixture 阈值门禁。
