...
13. 私有 blind pack 必须位于仓库外、使用 schema-v2 JSONL、至少包含 20 条冻结样本；`request` 只允许 `q/context`，检索 mode、model、limit 与 auto-build 由 runner 固定，Recall@3、候选资格和 answer-quality 标签各覆盖至少 25%。各行 `evaluation_date`、唯一 ID/查询、固定 categories 和独立 expected 标签继续 fail closed。fresh/stale 必须分别绑定不同 pack 与 snapshot；runner 清空模型凭证、阻断 socket/DNS、只用临时 SQLite，并在执行前后复核 pack、policy 与事实 JSON hash。schema-v4 baseline 保存样本数、cohort、输入/执行源码清单/subject hash、policy hash、开始时间、聚合类别、聚合指标及精确 numerator/denominator；policy 必须在 baseline 开始前冻结。必需分母为 `1..case_count`，route、freshness、hard violation 和 Top-1 coverage 分母必须等于 `case_count`。题目、标签、回答、仓库级失败和路径不得进入 baseline 或最终汇总。人审文件必须逐项绑定实际 Top-1，拒绝 pending/abstain/缺项/重复；冻结 policy 必须同时约束 Candidate Recall、Top-1 Acceptance/coverage、route、answer-quality、freshness 与 hard violation。检查器只证明外部 JSON 自洽，输出固定 `self_attested`、`accepted=false`；真实独立性仍需仓库外治理证据。全部 `blind_*` artifact 禁止进入公开 fixture 阈值门禁或 CI artifact。
