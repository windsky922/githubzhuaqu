...
通用教学事实闸门位于模型输出与公开 guidance 之间，正常和 SSE 共用同一分类器。它不依赖单一 `owner/repo` 正则，而是分别识别仓库实体/URL、命名框架、版本、Star、许可证和时效性项目声明；流式路径对累计内容先检查再展示当前 delta，阻止拆分 token 绕过。命中时整段通用 guidance 降为固定 limitation，`general_knowledge_disclosed=false`，具体项目部分仍只能经过 RAG 的 freshness、eligibility 与 claim 闸门。
...
本文档记录当前安全与确定性闸门完成后的实际实现架构。

## 提交级质量检查

质量 job 除构建、测试和安全检查外，还在临时工作目录运行六套离线固定 evaluator。版本化阈值配置先核验公开 fixture 的 SHA-256，再比较核心 retrieval/recommendation、追问路由、主张支持和约束安全指标；结果、commit 与 hash 作为短期 CI artifact 保存。私有 blind 与 push/PR CI 隔离：`evaluate_blind_rag.py` 对仓库外 schema-v2 pack 和冻结 snapshot 分别运行 fresh/stale cohort，要求关键标签至少 25% 覆盖，并绑定预冻结 policy；runner 固定检索参数、禁用模型/网络并在临时 SQLite 执行 contextual normal/SSE。schema-v4 baseline 只保留外部输入与执行源码清单 hash、聚合指标、精确 numerator/denominator、Top-1 subject commitment 和固定错误码。`check_blind_rag_acceptance.py` 再校验两个 cohort、policy 与完整人工判断，但其结果始终标记 `self_attested`、`accepted=false`；没有仓库外预提交或签名治理时，JSON 自洽不等于审查者独立。公开阈值检查器拒绝全部 `blind_*` artifact。完整流程见 `docs/private-blind-acceptance.md`。
...
生产 Ask 仅在 `GITHUB_WEEKLY_SNAPSHOT_ROOT` 指向具有效 freshness attestation 的 weekly 快照时执行检索；缺失、无效或不一致则返回 `unknown` 和规则拒答，绝不静默降级到 checkout 数据。完整 Ask final 由服务端生成不透明 `decision_id`，私有 SQLite 仅记录该次决策的规范化查询、快照身份、约束版本、freshness 与候选排名；管理员反馈通过该 ID 关联。assistant answer、prompt context、provider 输出、请求头和密钥均不写入三张 query 表，公开归档投影也不读取它们。
