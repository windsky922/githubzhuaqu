...
2. 确认现有 V1 已具备只读接口、最小状态、候选作用域、证据闸门、显式模型降级和最小本机历史，但知识型上一回答没有结构化提纲或指代映射；确定性诊断中“把第三点展开”由首轮 `knowledge` 落为 `clarify`。
3. V7 顺序固定为：知识多轮连续性 → 教学链与项目 RAG 解耦及 SSE 恢复 → 混合意图约束保留与教学事实闸门 → 本机 readiness/真实 Kimi 可选验收 → 独立 blind 与 Top-1 人工判断。
4. 公开只读部署暂不作为个人本机目标的前置条件；main 分支保护继续作为单人开发阶段的已接受风险，不在本阶段启用。
5. 新增 V7 审查路线图与交接摘要，并把 README 当前入口切换到 V7；补齐 `AGENTS.md` 完整验证中遗漏的 capability scope evaluator。

验证边界：本轮审查以静态代码、既有测试正文、附件运行证据和一次确定性编排诊断为主；未调用真实 Kimi、未读取真实凭证或运行态 SQLite，未执行独立 private blind、真实用户研究或完整验证矩阵。`output/`、`tmp/` 未触碰。
...
2. 按模块拆分复核：首批 76 tests 用时 55.207s、API 11 tests 用时 30.284s、评估/weekly/workflow 24 tests 用时 12.796s、其余 196 tests 用时 25.511s，均通过。
3. 结论：当前完整 Python 基线约 124 秒，应为调用时限边界而非功能测试卡死；后续完整验证应保留分组证据，或将单次执行上限调高后再宣称 discover 全绿。

验证：分组累计 307 tests 通过；`git diff --check` 通过。未修改产品代码、`output/`、`tmp/`、SQLite 或归档。
...
1. 新增 `docs/project-review-agent-v5-roadmap.md`：以当前 `bb1e249` 基线重新审查生产快照、blind 主链、浏览器留存、查询反馈和分支治理，明确 P0-19 至 P2 的风险顺序与验收。
2. 新增 `docs/project-review-agent-v5-handoff.md`：记录当前已证实/未证实事项、必读顺序、只读基线命令、P0-19 首任务、完整验证和可复制启动提示。
3. README 改为优先链接 V5 审查和 V5 交接，保留 V4 作为历史取证入口。

### 2. 审查边界

1. 本轮仅审查受控代码、测试、工作流、文档和已有 CI 记录；未读取 `output/`、`tmp/`、运行态 SQLite、真实 blind pack、真实生产快照或历史 blob。
2. GitHub 分支保护和 Issue 创建仍受当前 CLI REST 401、连接器 403 阻塞；路线图不把 runbook 当作规则已经生效的证据。
...
1. 在 `AGENTS.md` 固化最多三名子 Agent 的启用条件、只读白名单、禁止访问范围、统一回报格式和主 Agent 唯一写入责任。
2. 新增 `docs/multi-agent-review-protocol.md`，定义 code-explorer、reviewer、test-analyst 的独立工作包、汇总规则、冲突裁决与实施后复核顺序。
3. README 增加协议入口，供后续窗口按相同边界复用。

### 2. 本次试运行结论

- 已进行三角色只读试运行：代码探索、风险审查与测试分析均认为项目模块边界和 CI 结构适合受限并行审查。
- 试运行未修改业务实现、未运行测试、未访问 `tmp/`、运行态数据、真实历史 blob 或外部网络；因此结论仅是静态审查，不等同于测试、远端验证或历史内容取证。
- 后续真实历史 SQLite 扫描、远端归档/Pages 验证和 Playwright 执行仍需单独授权并由主 Agent 串行处理。

### 3. 验证与范围

- 本阶段只改动协作规范和文档入口；不触碰未跟踪的 `tmp/`、`output/`。
- 后续对协议文本执行 Markdown/链接存在性、`git diff --check` 与工作区范围检查；不把文档检查表述为完整应用回归。

## 2026-07-15 追加：P0-1 历史公开归档脱敏取证工具

### 1. 修改内容

1. 新增独立历史内容审计器：路径枚举与最新 tree attestation 保持在原有 `audit_public_archive.py`；结构审计必须显式确认后才会下载数据库 blob，并只输出 magic、SHA、提交范围、表/列名、行数、规范化时间范围和风险类别。
2. 原始 blob 只落入系统临时目录并逐个删除；脱敏 JSON 只能写入未跟踪 `tmp/archive-audit/`，拒绝写入公开目录或工作区外位置。
3. 本阶段只实现与测试工具，不运行 `--confirm-structure-scan`，不下载历史 SQLite、不改写历史、不执行缓存清理、凭据轮换或通知。

### 2. 边界

1. `--dry-run` 仍只做路径和 blob SHA 枚举；结构扫描无 `GITHUB_TOKEN` 时在下载前安全失败，且不会要求或输出凭据。
2. 结构报告中的风险类别只说明字段命名和表结构风险，不能代替对真实内容或已公开范围的最终判断。

### 3. 验证记录

1. 本地完整验证通过。首次 CI `29408400738` 暴露测试假设 runner 已存在 `tmp/` 的问题；已在 `tests/test_audit_public_archive_content.py` 中显式创建私有临时根目录。
2. 修复提交 `fe68a02` 的 CI `29408568021` 三项 job 全部成功；在后续单独授权前，未执行真实 `--confirm-structure-scan`。
3. 首次经授权的真实扫描在 GitHub blob 返回带换行的 base64 包装时于解码前失败；未写入临时数据库或报告。已改为仅移除 base64 包装空白并保持严格校验，新增回归测试；真实扫描将在该修复经 CI 验证后重试。

### 4. 经授权的结构扫描结果

1. 修复提交 `15d36e5` 的 CI `29409391777` 三项 job 全部成功后，使用一次性本机 CLI 认证执行历史结构扫描。扫描 39 个可达提交，发现 35 个不同数据库 blob、38 次历史出现，路径类别为 1；全部通过 SQLite magic 和只读结构检查。
2. 报告共统计 768 个历史表结构实例、105000 行快照计数和 403 组时间范围；全局规范化时间范围为 1970-01-01 至 2026-07-13。行数为各历史快照之和，不能视为独立用户记录数。
3. 按表/列名称得到的结构风险类别计数为：凭据命名 27、用户输入命名 83、通知/订阅 45、任务运行态 284。该统计不读取、输出或保存字段值，不能单独证明存在真实敏感内容。
4. 原始 blob 仅在系统临时目录逐个处理并已随扫描结束删除；脱敏报告仅位于未跟踪 `tmp/archive-audit/`，未提交、未发布、未改写历史。

## 2026-07-15 追加：P0-13 公开归档修复发布已执行

### 1. 实际结果

1. 经用户明确授权后，手动运行“公开归档修复发布” workflow：run `29406619346` 成功完成恢复公开历史、字段投影验证、Pages 重建、公开归档发布和远端 tree attestation；未运行正常 weekly 采集、Kimi 或通知发送。
2. 独立只读复核 `weekly-archive` HEAD 为 `a9447d3259f40a06111824f6105144252233847e`：最新 tree 共 154 个文件，禁止路径为 0。Pages `projects.json` 返回 HTTP 200，根路径为正常重定向。

### 2. 未关闭事项

1. 本结果只证明最新公开 tree 已不含禁止路径，不会删除历史 commit、fork 或缓存中的旧 SQLite；历史内容级取证和是否处置历史仍需单独授权。
2. 已生成 `docs/context-handoff-p0-13-archive-remediation.md`，供下一阶段按最小脱敏范围继续历史取证或其它 P0 工作。
...
1. `weekly-archive` 发布器不再把允许目录中的 JSON 原样复制：本地 `data/raw`、`data/selected`、`data/runs`、`data/trends` 仍是完整事实来源，写入公开 worktree 前按固定字段白名单重新构造 JSON。
2. 项目记录保留 SQLite 重建和 Pages 所需的公开仓库、评分、趋势、README 摘要与质量字段；不发布 `readme_excerpt`、未知字段或嵌套任意 payload。运行摘要只保留公开计数、状态、报告链接和汇总指标，移除查询、错误详情、运行路径、状态路径和投递结果。趋势条目也按名称/计数或公开项目摘要缩小嵌套字段。
3. 新增临时 worktree 回归：私有 `query`、`note`、`payload`、查询列表、错误详情和投递结果 canary 不会进入公开文件或暂存树；若敏感标记位于已允许公开字段，现有暂存扫描仍会 fail closed。

### 2. 边界

1. 本阶段不改写本地 JSON 事实来源、不触发 weekly workflow 或真实外发，也不修改 `weekly-archive` 历史；下一次受控周报发布才会把字段投影应用到远端最新 tree。
2. 字段白名单降低应用运行态泄露面，但不是对公开仓库描述和 README 摘要的通用内容脱敏承诺；已允许字段仍继续接受 staged-tree 敏感标记扫描。

## 2026-07-15 追加：P0-11B 安全流式输出与引用闸门

### 1. 修改内容

1. 流式 Ask 改为先缓冲 provider 输出并完成质量校验，再发送既有 `meta → delta* → final`；质量失败、无效引用、未知仓库或危险指令时不发送任何 provider delta。
2. 普通 POST 也不再自动补造引用标记；缺失引用按既有规则降级，避免“回答末尾存在编号”被误当作真实引用。
3. 新增跨 delta 危险指令回归，验证未经校验内容不会进入 SSE draft。

### 2. 边界

1. 本阶段保持 Ask/SSE 字段、事件名、事件顺序和无状态追问不变；不新增服务端会话、不调整检索权重。
2. `evidence_relevance`、`claim_support`、`data_freshness` 仍是后续字段级证据阶段，不将其伪装成已完成。

## 2026-07-15 追加：P0-12 远端公开归档取证与 attestation

### 1. 修改内容

1. 新增 `scripts/audit_public_archive.py`：通过 GitHub API 只读枚举 `weekly-archive` 最新 tree，拒绝数据库、WAL/SHM、密钥和日志类路径；可选按历史提交统计首次/最后出现位置，不下载 blob 内容。
2. 周报 workflow 在两次归档发布完成后执行最新 tree attestation，禁止本地发布成功却远端公开 tree 仍含禁止文件。
3. 首次远端历史树取证扫描 38 个提交：`data/github_weekly.sqlite` 在全部 38 个提交中存在，首次可达提交为 `96149b9b031fce4304fc43759776384e95958ab2`，当前 HEAD 为 `54677724942c3a29fbeb28eaa810b0fb03f6901d`。本阶段未下载数据库、未改写历史、未轮换凭据。

### 2. 边界

1. 该 attestation 只证明最新 tree 的文件路径，不证明允许 JSON/Markdown 的字段内容已脱敏；字段级公共投影留待后续阶段。
2. 历史数据库仍可从旧 commit 获取；是否重写历史、清理缓存或轮换凭据必须根据后续脱敏审计和用户授权单独决定。
...
为下一开发窗口提供可直接执行的工程交接包，避免重新追溯长对话或把 V3 路线图中的风险结论误当成已经实现的能力。

### 2. 交接内容

1. 新增 `docs/project-review-agent-v3-handoff.md`，记录当前 `4d5c1a0` 基线、阅读顺序、已完成阶段、固定评估结果、关键风险、代码地图、环境变量、完整验证和可复制启动提示。
2. 将下一任务明确为 P0-11A：先只读分类 `data/` 与远端归档，再建立公共 allowlist；原始 SQLite、用户状态、未知文件和测试敏感 canary 不得进入 `weekly-archive`。
3. 明确运行态持久化选择：无真实状态时优先从公共 JSON 重建派生 SQLite；存在真实订阅、反馈或任务时，必须先让用户在无状态重建和私有持久化之间做知情选择。
4. 保留已知 Git 对象损坏、未校验流式 delta、顶层回答类型缺失、旧语料与数据滞后等风险，不在交接任务中擅自修复或掩盖。

### 3. 边界

本次只新增交接文档与入口，不修改 API、SQLite schema、Prompt、前端、发布器、工作流、检索权重或模型配置；不触碰 `tmp/`。下一窗口仍需先核验远端状态和工作区改动，不能把本文中的提交号或远端内容当作永久不变事实。

## 2026-07-13 追加：GitHub 项目研究 Agent V3 第一性原理对抗性审查

### 1. 审查基线与范围

以本地和远端一致的 `fea50ce` 为代码基线，复核当前代码、Prompt、API/SQLite 契约、React 行为、V1/V2 路线图、四套固定评估、Python/Vitest/Playwright 回归、本地 SQLite、语料版本、远端归档和 Git 对象完整性；未调用真实 Kimi、GitHub 采集或任何真实外发。

### 2. 主要结论

1. 眼下最没有把握的是由实现者共同维护的固定评估集能否代表真实用户表达；当前满分项只能视为回归通过，不能证明未见表达和多轮指代已经校准。
2. 最大的产品机制遗漏是检索前没有统一的“用户意图、回答类型、检索范围、实际查询和证据策略”决策层，学习、解释、比较和项目搜索仍可能混入同一 `new_search` 路径。
3. 用户容易忽略的风险是模型原始 delta 在最终质量校验前已经显示，后续规则版 final 无法撤回草稿；同时公共归档发布器会整包复制运行态 `data/`，需要立即建立公共数据白名单投影。
4. 本地活跃语料仍为 `legacy-v0` 且来源截至 2026-06-03；远端归档更新更晚，存在“代码能力已实现、当前数据尚未启用”的状态差距。

### 3. 产出与验证

新增 `docs/project-review-agent-v3-roadmap.md`，在 README 顶部增加 V3 入口并保留 V2/V1 历史入口。报告严格区分【事实】【推断】【待验证】，给出 P0/P1/P2 顺序和可执行验收标准。审查基线复算中 Python 242 项、Vitest 11 项、mock Playwright 12 项、真实后端 Playwright 6 项均通过，四套固定评估保持既有结果；文档提交前再次执行完整检查。

### 4. 边界

本次仅新增审查文档和入口，不修改 API、SQLite schema、Prompt、前端行为、检索权重或模型配置；不触碰 `tmp/`。本地 full fsck 发现一个不在可达历史中的损坏 loose object，修复前必须备份，不能在本次文档提交中直接删除。
...
明确删除旧 `admin_token` 地址栏参数只能阻止未来传播，不能撤销浏览器历史或服务器、代理、CDN 已记录的请求，避免用户误以为旧口令已经安全清除。

### 2. 修改内容

1. 四个管理页面检测到旧参数时继续只判断参数存在并删除地址栏内容，不读取、显示或迁移参数值。
2. 共享认证组件明确提示旧口令可能已进入访问日志，并要求立即轮换 `ADMIN_API_TOKEN`。
3. README、API 和架构文档同步区分“阻止未来传播”与“撤销历史泄露”，页面生成及 Playwright 增加回归断言。

### 3. 边界

代码不会也无法自动轮换真实环境变量；若旧 URL 曾包含真实口令，轮换必须由管理员在环境配置中完成。FastAPI 鉴权契约、SQLite、Ask、SSE 和外发确认机制均不改变。
...
RAG 检索评估已经能由 workflow 自动写入 jobs，并且后端提供了 `/v1/rag/search-evaluation-trends` 趋势接口。为让本地管理首页直接看到这些质量信号，本次把趋势接口接入 `admin.html?api=1` 的 RAG 区域。

### 2. 修改内容

1. `admin.html` 新增 `ragEvaluationTrends` 展示区域。
2. 本地 API 模式下读取 `/v1/rag/search-evaluation-trends?limit=8`。
3. 页面展示评估任务数、平均样本、平均零命中、最新推荐模式、推荐模式分布、趋势建议和最近评估任务。
4. 静态 Pages 模式下保留中文提示，不误调用本地后端。
5. 更新 README、API 文档和页面构建测试。

### 3. 后续空间

后续可以在管理首页增加“创建检索评估 planned 任务”按钮，但当前先只做趋势可视化，避免把查看功能和执行功能混在一起。
