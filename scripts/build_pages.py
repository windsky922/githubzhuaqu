...
<body>
  ...
  <script>
    ...
    const ragChatControls = {
      query: document.getElementById("ragChatQuery"),
      mode: document.getElementById("ragChatMode"),
      limit: document.getElementById("ragChatLimit"),
      language: document.getElementById("ragChatLanguage"),
      category: document.getElementById("ragChatCategory"),
      source: document.getElementById("ragChatSource"),
      sendButton: document.getElementById("sendRagChat"),
      clearButton: document.getElementById("clearRagChat"),
    };
    const ragChatState = { messages: loadRagChatHistory() };
    ...
    function setupRagChat() {
      renderRagChat();
      if (!shouldUseApi()) {
        ragChatControls.sendButton.disabled = true;
        ragChatStatus.textContent = "RAG 对话需要本地后端或 api=1 模式。";
        return;
      }
      ragChatStatus.textContent = "每轮独立调用 /v1/rag/ask；历史只保存在当前浏览器。";
    ...
    function sendRagChatMessage() {
      const question = ragChatControls.query.value.trim();
      if (!question) {
        ragChatStatus.textContent = "请输入 RAG 对话问题。";
        return;
      }
      const params = new URLSearchParams();
      const mode = ragChatControls.mode.value || "hybrid";
      const limit = Math.max(1, Math.min(Number(ragChatControls.limit.value || 5), 30));
      params.set("q", question);
      params.set("mode", mode);
      params.set("limit", String(limit));
      if (mode === "vector" || mode === "hybrid") params.set("auto_build", "true");
      if (ragChatControls.language.value.trim()) params.set("language", ragChatControls.language.value.trim());
      if (ragChatControls.category.value.trim()) params.set("category", ragChatControls.category.value.trim());
      if (ragChatControls.source.value.trim()) params.set("source", ragChatControls.source.value.trim());
      ragChatControls.sendButton.disabled = true;
      ragChatStatus.textContent = "正在生成证据约束回答...";
      fetch(`/v1/rag/ask?${params.toString()}`, { cache: "no-store" })
        .then(jsonOrThrow)
        .then(data => {
          const message = {
            id: `rag-chat-${Date.now()}`,
            asked_at: new Date().toISOString(),
            question,
            mode,
            limit,
            language: ragChatControls.language.value.trim(),
            category: ragChatControls.category.value.trim(),
          ...
          ragChatStatus.textContent = "已生成回答。";
        })
        .catch(error => {
          const message = {
            id: `rag-chat-${Date.now()}`,
        ...
        .finally(() => {
          ragChatControls.sendButton.disabled = false;
        });
    }
    ...
    function ragChatResponseSnapshot(data) {
      ...
      return {
        ...
        prompt_context: data.prompt_context || "",
        source_explanation_id: data.source_explanation_id || "",
      };
    }

    function loadRagChatHistory() {
      try {
        const raw = window.localStorage.getItem("github_weekly_rag_chat_history");
        const parsed = raw ? JSON.parse(raw) : [];
    ...
    function saveRagChatHistory(messages) {
      window.localStorage.setItem("github_weekly_rag_chat_history", JSON.stringify((messages || []).slice(0, 20)));
    }

    function clearRagChatHistory() {
      ragChatState.messages = [];
      window.localStorage.removeItem("github_weekly_rag_chat_history");
      renderRagChat();
      ragChatStatus.textContent = "已清空本地 RAG 对话历史。";
    }
