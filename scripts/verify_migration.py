...
def _json_counts(root: Path) -> dict[str, int]:
    return {
        ...
        "project_corpus": sum(len(_read_json_list(path)) for path in _json_files(root / "data" / "selected")),
        "project_corpus_fts": sum(len(_read_json_list(path)) for path in _json_files(root / "data" / "selected")),
